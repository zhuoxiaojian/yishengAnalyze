...
class AdminSplitDateTime(forms.SplitDateTimeWidget):
    ...
    def render(self, name, value, attrs=None):
        # print(super(AdminSplitDateTime, self).render(name, value, attrs))
        ...
        if len(input_html) > 1:
            ...
            input_html_new = [ht for ht in super(AdminSplitDateTime, self).render(name, value, attrs).split('/><') if ht != '']
            if len(input_html_new) > 1:
                input_html_new[0] = input_html_new[0] + "/>"
                input_html_new[1] = "<" + input_html_new[1]
            return mark_safe(
                '<div class="datetime clearfix"><div class="input-group date bootstrap-datepicker"><span class="input-group-addon"><i class="fa fa-calendar"></i></span>%s'
                '<span class="input-group-btn"><button class="btn btn-default" type="button">%s</button></span></div>'
                '<div class="input-group time bootstrap-clockpicker"><span class="input-group-addon"><i class="fa fa-clock-o">'
                '</i></span>%s<span class="input-group-btn"><button class="btn btn-default" type="button">%s</button></span></div></div>' % (
                input_html_new[0], _(u'Today'), input_html_new[1], _(u'Now')))
